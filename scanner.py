...
def escanear_puertos(host, puertos, salida_text, barra_progreso, progreso_label):
    ...
    cola.join()
    salida_text.insert(END, "\n✅ Escaneo de puertos TCP terminado.\n")

def escanear_puertos_udp(host, puertos, salida_text, barra_progreso, progreso_label):
    resultados.clear()
    ...
    for port in puertos:
        try:
            ...
            try:
                ...
                with lock:
                    salida_text.insert(END, f"🟢 UDP {port} → respuesta: {data.decode(errors='ignore').strip()}\n")
                    salida_text.see(END)
            except socket.timeout:
                pass  # No mostrar nada si no hay respuesta
            sock.close()
        except:
            pass  # No mostrar errores como WinError 10054

        completados += 1
        barra_progreso["value"] = (completados / total) * 100
        progreso_label.config(text=f"{int((completados / total) * 100)}%")

    salida_text.insert(END, "\n✅ Escaneo de puertos UDP terminado.\n")

def iniciar_escaneo_udp(ip_entry, puerto_entry, salida_text, barra_progreso, progreso_label):
    ...
    threading.Thread(target=escanear_puertos_udp, args=(host, puertos, salida_text, barra_progreso, progreso_label), daemon=True).start()
...
def main():
    ...
    boton_frame = Frame(ventana)
    boton_frame.pack(pady=10)

    Button(boton_frame, text="Escanear Puertos", command=lambda: iniciar_escaneo_puertos(ip_entry, puerto_entry, salida, barra_progreso, progreso_label)).pack(side=LEFT, padx=5)
    Button(boton_frame, text="Escanear UDP", command=lambda: iniciar_escaneo_udp(ip_entry, puerto_entry, salida, barra_progreso, progreso_label)).pack(side=LEFT, padx=5)
    Button(boton_frame, text="Guardar Resultados", command=lambda: guardar_resultado(salida)).pack(side=LEFT, padx=5)
    Button(boton_frame, text="🧹 Limpiar Resultados", command=lambda: limpiar_resultados(salida)).pack(side=LEFT, padx=5)
